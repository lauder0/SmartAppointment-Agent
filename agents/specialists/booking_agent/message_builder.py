...
class MessageBuilder:
    ...
    def create_appointment_success_message(
        self,
        tech: Dict[str, Any],
        appointment_history: Dict[str, Any] | None = None,
        start_time=None,
        end_time=None
    ) -> str:
        """创建预约成功消息"""
        ...
        if appointment_history:
            ...
            if project and project != "未知":
                detail_parts.append(f"项目：{project}")
        ...
        if start_time and end_time:
            try:
                ...
                detail_parts.insert(0, f"时间：{date_text} {start_text}-{end_text}")
            except Exception:
                pass

        detail_text = ""
        if detail_parts:
            detail_text = "预约信息：" + "，".join(detail_parts) + "。"
