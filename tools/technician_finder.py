...
class TechnicianFinder:
    ...
    @staticmethod
    def _normalize_excluded_ids(excluded_technician_ids: list | None) -> set[int]:
        normalized: set[int] = set()
        for value in excluded_technician_ids or []:
            try:
    ...
    def find_specific_technician(
        self,
        technician_name: str,
        start_time: datetime,
        end_time: datetime,
        yield_func: Optional[Callable] = None,
        excluded_technician_ids: list | None = None,
    ) -> Optional[Dict]:
        """查找指定技师的可用性"""
        # 通过Services层访问数据库
        from services.appointment_service import AppointmentService
        appointment_service = AppointmentService()
        excluded_ids = self._normalize_excluded_ids(excluded_technician_ids)
        ...
        if specific_tech:
            if int(specific_tech["id"]) in excluded_ids:
                if yield_func:
                    yield_func(f"[THOUGHT][预约机器人] {technician_name}技师已被用户要求更换，跳过\n")
                return None
    ...
    def find_similar_available_technician(
        self,
        target_technician: Dict[str, Any],
        start_time: datetime,
        end_time: datetime,
        yield_func: Optional[Callable] = None,
        excluded_technician_ids: list | None = None,
        ...
        other_techs = [
            ...
            if tech["id"] != target_technician["id"] and int(tech["id"]) not in excluded_ids
        ]
        if not other_techs:
            return None
    ...
    def find_available_technician(
        ...
        for tech in filtered_techs:
            if int(tech["id"]) in excluded_ids:
                continue
    ...
    def find_technician_with_thought(self, appointment_history: Dict[str, Any], 
        ...
        excluded_technician_ids = appointment_history.get("excluded_technician_ids") or []
        ...
        if technician_name and technician_name != "未知":
            specific_tech = self.find_specific_technician(
                technician_name,
                start_time,
                end_time,
                yield_func,
            ...
            if target_tech:
                similar_tech = self.find_similar_available_technician(
                    target_tech,
                    start_time,
                    end_time,
                    yield_func,
        ...
        # 查找可用技师
        return self.find_available_technician(
            filtered_techs,
