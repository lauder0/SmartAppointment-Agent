...
class UserBehaviorRepository(BaseUserBehaviorRepository):
    ...
    def get_known_user_ids(self, days_back: Optional[int] = None) -> List[str]:
        """Return user ids that have behavior records."""
        with self.session_manager.session_scope() as session:
            query = session.query(UserBehavior.user_id).distinct()
            if days_back:
                cutoff_date = utc_now_naive() - timedelta(days=days_back)
                query = query.filter(UserBehavior.created_at >= cutoff_date)
    ...
    def update_user_preference(
        ...
        Returns:
            更新是否成功
        """
        if preference_value in (None, ""):
            return False

        try:
            increment = max(1, int(confidence_score))
        ...
        with self.session_manager.session_scope() as session:
            ...
            if existing:
                # 增加置信度
                existing.confidence_score += increment
    ...
    def _recommendation_to_dict(self, recommendation: UserRecommendation) -> Dict[str, Any]:
        ...
        return {
            ...
            'payload_json': recommendation.payload_json,
